...
class GeneticCycleChoose():
    def __init__(self, pop_size, num_generations, panel, cycle_key, threads = 128):
    ...
    def Comb_spearmanr(self, genes_perm):
        ...
        CHANNEL_ORDER = {
            "AF488": genes_perm[[0,1,2,3]],
            "AF546": genes_perm[[0,4,5,6]],
            "AF594": genes_perm[[1,4,7,8]],
            "AF647": genes_perm[[2,5,7,9]],
            "AF750": genes_perm[[3,6,8,9]]
        }
            
        
        comb_R = 0
        
        for tmp_comb in CHANNEL_ORDER.values():
...
if __name__ == "__main__":
    ...
    panel = np.load("best_perm_110.npy")
    ...
    for key,value in CYCLES.items():
        ...
        ga = GeneticCycleChoose(pop_size = pop_size, num_generations = num_generations, cycle_key = key, panel = value, threads = 128)
        best_perm = ga.genetic_algorithm()
        np.save(f"./best_perm_110_{key}.npy", best_perm)
