...
def multiCycleRegister(Para):
    ...
    DAPI_file = sorted(glob.glob(os.path.join(Para.output, f"Registration/unstitched/{Para.Anchor_Round}_DAPI/*AF405.tif")))
    xml_file = sorted(glob.glob(os.path.join(Para[Para.Anchor_Round], "*_info.xml")))[0]
    ...
    print(info)  
    Parallel(n_jobs = -1, backend='loky')(delayed(_tile_stitching)(rd_ch, Para, STITCHED_IMAGE, STITCHED_IMAGE_SIZE, GLOBAL_GRID) for rd_ch in rds_chs)
