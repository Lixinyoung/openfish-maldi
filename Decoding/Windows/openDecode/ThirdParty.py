...
def runRSFISHforTiles(Para, anisotropyCoefficient = 1.00,
    ...
    imageJ = "E:/Fiji.app/ImageJ-win64.exe"
...
def runCellpose(Para, restore_type = 'denoise_cyto3', diameter = 40, flow_threshold=1., cellprob_threshold=-6.):
    ...
    model = denoise.CellposeDenoiseModel(gpu=True, model_type="cyto3",restore_type=restore_type, chan2_restore=True)
    
    #model = denoise.CellposeDenoiseModel(gpu=True, model_type="cyto3",restore_type="denoise_cyto3", chan2_restore=True)
    ...
    cells = Parallel(n_jobs=40, backend='loky')(delayed(_getContour)(cell_id, mask) for cell_id in range(1,max_cell+1))
    ...
    cells = Parallel(n_jobs=40, backend='loky')(delayed(_smoothen_cell)(cell, smooth_radius, tolerance) for cell in cells)
...
def runBaysor(Para, min_molecules_per_cell = 5, count_matrix_format = "loom", prior_segmentation_confidence = 0.3, scale = 30, scale_std = "50%"):
    ...
    baysor_path = "D:/software/baysor/baysor.cmd run"
...
def runStardist_and_Cellpose(Para, prob_thresh = 0.5, nms_thresh = 0.4, trained_model = '2D_versatile_fluo', sigma = 2.5,
                            restore_type = 'denoise_cyto3', diameter = 40, flow_threshold=1., cellprob_threshold=-6.):
