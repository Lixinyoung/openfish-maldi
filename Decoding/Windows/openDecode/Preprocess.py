...
def tilePreprocess(Para):
    ...
    Ref_filepath = glob.glob(os.path.join(Para[Para.Anchor_Round], "*ORG.tif"))
    
    for rd, channels in Para.Round_channel.items():
        
        filepath = glob.glob(os.path.join(Para[rd], "*ORG.tif"))
        ...
        for m in tqdm(Para.tile_numbers, total = len(Para.tile_numbers), 
            ...
            for im in tmp_tile_path:
                ...
                # channel_mask = image_in > Para.channel_clip[channel] * 0.9
        ...
        if Para.BaSiC:
            for ch in tqdm(channels, desc = f"Preprocessing(BaSiC) {rd} tiles"):
                ...
                if ch == "DAPI" or ch == 'AF405':
                    ...
                    tmp_tile_path = glob.glob(os.path.join(Para.output, f"Registration/unstitched/{rd}/*{ch}.tif"))
    ...
    if Para.rRNAseg:
        ...
        filepaths = glob.glob(os.path.join(Para.rRNA, "*ORG.tif"))
    ...
    if Para.extraseg:
        ...
        filepaths = glob.glob(os.path.join(Para.extra, "*ORG.tif"))
