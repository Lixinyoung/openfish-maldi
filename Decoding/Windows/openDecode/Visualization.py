...
def plotXenium(coord, Para, stardist_roi):
    ...
    if isinstance(cells, str):
        ...
        adata = sseg.aggregate.count_transcripts(sdata, gene_column = "gene", shapes_key=shapes_key, points_key='dist_coord')

        adata.obs["region"] = shapes_key
        adata.obs["region"] = adata.obs["region"].astype("category")
        adata.obs["Name"] = adata.obs_names
        adata.obs["x"] = sdata[shapes_key].apply(lambda x: x.centroid.x)
        adata.obs["y"] = sdata[shapes_key].apply(lambda x: x.centroid.y)
...
def _MultiModel_Shape(Para, scaler = 0.325, stardist_roi = True):
    ...
    # baysor input geometry can be invalid.
