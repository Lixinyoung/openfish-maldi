...
class Matrixization():
    ...
    def _prepare_shapes(self):
        ...
        try:
            try:
                ...
                polygon_list = np.load(os.path.join(self.para.OUTPUT_PATH, 'Segmentation/StarDist/RoiSet.npy') ,allow_pickle=True)
                cells = gpd.GeoDataFrame({"geometry":polygon_list})
    ...
    def _prepare_tables(self, adata, sdata, shapes_key: str = 'cell_boundaries'):
        ...
        if len(adata.var_names) > 60 or self.para.run_basic_clustering:
            ...
            adata.var["FP"] = adata.var_names.str.startswith(("FP", "sFP", 'FalsePositive'))
            ...
            adata_back.obs['leiden'] = 'Unassigned'
    ...
    def matrixization(self, force = False):
        ...
        if shapes:
            ...
            if not PROGRESS_DICT['matrixization']['assign_transcripts'] or force:
                ...
                # adata = sseg.aggregate.count_transcripts(sdata, gene_column = "gene", shapes_key=shapes_key, points_key='transcripts')
