...
class Stitching_Registration():
    ...
    def _runMIST(self):
        ...
        if not czi.is_mosaic():
            ...
            rows = np.array(starty) // round(self.para.tile_size * 0.9) + 1
            cols = np.array(startx) // round(self.para.tile_size * 0.9) + 1
    ...
    def _EulerTransform(self, fix: sitk.SimpleITK.Image, move: sitk.SimpleITK.Image) -> dict:
        ...
        param_map['NumberOfSpatialSamples'] = [f'{self.para.tile_size}']
    ...
    def _applyEulerTransform2048(self, theta: float, dy: float, dx: float, move: sitk.SimpleITK.Image) -> sitk.SimpleITK.Image:
        ...
        transform.SetCenter([self.para.tile_size / 2,self.para.tile_size / 2])
    ...
    def stitching_and_registration(self, force = False):
        ...
        self.STITCHED_IMAGE_SIZE = (
            self.GLOBAL_GRID["y_pos2"].max() + self.para.tile_size,
            self.GLOBAL_GRID["x_pos2"].max() + self.para.tile_size,
        )
        ...
        if self.para.EXTRA:
            ...
            if 'CytoRNA' in name_list:

                morphology_CytoRNA = np.zeros((self.STITCHED_IMAGE_SIZE[0], self.STITCHED_IMAGE_SIZE[1], 3), dtype=np.uint16)
                    ...
                    if ch != 'SKIP':
                        ...
                        if name == 'CytoRNA':
                            if ch == self.para.ANCHOR_CHANNEL:
                                morphology_CytoRNA[:,:,0] = stitched_image
                            else:
                                morphology_CytoRNA[:,:,2] = stitched_image
            ...
            if 'CytoRNA' in name_list:
                tifffile.imwrite(os.path.join(self.para.OUTPUT_PATH, f"Registration/morphology_CytoRNA.tif"), morphology_CytoRNA)
