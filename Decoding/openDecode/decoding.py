...
class Decoding():
    ...
    def __init__(self, para, dist_threshold: float = 1.0, prob_threshold: float = 1.0, prob_diff_threshold: float = 0.5, qv_ratio_threshold: float = 0.85):
    ...
    def _all_single_decode(self, rd: str):
        ...
        for ch in channel:
            ...
            spots['qv'] = 0.4 + (spots['intensity'] - spots['intensity'].min()) / (spots['intensity'].max() - spots['intensity'].min()) * (0.6)
    ...
    def decoding(self, force = False):
        ...
        Final_df['qv'] = Final_df['qv'] * 20 / (1 - self.prob_diff_threshold) + (40 - 20 / (1 - self.prob_diff_threshold))
