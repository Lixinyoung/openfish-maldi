...
def _run_single_spotiflow(img,
    ...
    df = df[df['threshold'] > intensity_threshold].copy()
    # df['intensity'] = quantile_transform(df[['prob']], n_quantiles=min(1000, len(df)), output_distribution='normal',  random_state=42, copy = False).flatten()
    df['intensity'] = zscore(df['prob'])
