...
class Matrixization():
    ...
    def _prepare_shapes(self):
        ...
        cells = np.load(os.path.join(self.para.OUTPUT_PATH, "Segmentation/Cellpose_nuclei/cellpose_polygon.npy") ,allow_pickle=True)
        ...
        if self.para.EXTRA and 'CytoRNA' in self.para.EXTRA_NAMES:
            ...
            cells = np.load(os.path.join(self.para.OUTPUT_PATH, "Segmentation/Cellpose_Cyto/cellpose_polygon.npy") ,allow_pickle=True)
            cells = gpd.GeoDataFrame({"geometry":cells})
    ...
    def matrixization(self, force = False):
        ...
        if shapes:
            ...
            points_distance_threshold = 2 * np.sqrt(shapes[shapes_key].geometry.area / np.pi).mean() * 0.325
        ...
        log.info(f'Fitering transcripts within distance {points_distance_threshold: .1f} um...')
